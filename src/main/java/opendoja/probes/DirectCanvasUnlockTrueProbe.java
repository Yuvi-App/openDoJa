...
/**
 * Verifies that direct Canvas loops which finish frames with unlock(true)
 * inherit the graphics-present sync cadence instead of presenting at host speed.
 */
public final class DirectCanvasUnlockTrueProbe {
    private DirectCanvasUnlockTrueProbe() {
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            if (avgGapMs < 10.0) {
                throw new IllegalStateException("unlock(true) is still presenting too fast: avg gap " + avgGapMs + " ms");
            }
            if (avgGapMs > 35.0) {
                throw new IllegalStateException("unlock(true) pacing drifted too slow: avg gap " + avgGapMs + " ms");
            }
        } finally {
            app.canvas.stopLoop();
            app.terminate();
    ...
    static final class ProbeCanvas extends Canvas implements Runnable {
        ...
        private final Thread loopThread = new Thread(this, "direct-canvas-unlock-true-probe");
